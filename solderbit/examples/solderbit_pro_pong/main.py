...
def draw_players():
    for i in range(3):
        game_screen.fb.line(2+i, player_one_pos, 2+i, player_one_pos+player_length, st7789.WHITE)
        game_screen.fb.line(125+i, player_two_pos, 125+i, player_two_pos+player_length, st7789.WHITE)


def draw_ball():
    for i in range(4):
        game_screen.fb.line(ball_x_pos+i, ball_y_pos, ball_x_pos+i, ball_y_pos+3, st7789.WHITE)
...
def start_state():
    game_screen.fb.text("Press start", 20, 20, st7789.WHITE)
    game_screen.fb.text("for a new game", 9, 30, st7789.WHITE)
    inputs = controller.read_all_inputs()
...
def game_over_state():
    ...
    if left_score > right_score:
       game_screen.fb.text("You win!", 23, 20, st7789.WHITE)
    else:
       game_screen.fb.text("Game over", 23, 20, st7789.WHITE)
...
def state_machine(state):
    game_screen.fb.fill(st7789.BLACK)
    out = state
    ...
    game_screen.fb.rect(0, 0, width, height, st7789.WHITE)
